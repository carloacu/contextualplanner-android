...
namespace {
    ...
    std::string _toString(JNIEnv *env, jstring inputString) {
      if (env == nullptr)
        return "";
      const char *cstring;
...
Java_com_contextualplanner_BnfParserKt_flattenBnfRegex(
        JNIEnv *env, jclass /*clazz*/,
        jstring textJStr) {
  auto text = _toString(env, textJStr);
  auto results = flattenBnfRegex(text);

  jobjectArray result;
  result = (jobjectArray) env->NewObjectArray(results.size(),
                                              env->FindClass("java/lang/String"),
                                              env->NewStringUTF(""));

  jsize arrayElt = 0;
  for (const auto& currRes : results)
    env->SetObjectArrayElement(result, arrayElt++,
                               env->NewStringUTF(currRes.c_str()));
  return result;
}